...
fn invalid_chunk_size() {
    vec![1, 2, 3, 4, 5, 6, 7].into_pointer_par_chunk_slice(2);
}
...
fn no_thread_unchecked() {
    let slice = vec![1, 2, 3, 4].into_pointer_par_chunk_slice(2);
...
fn single_thread_unchecked() {
    ...
    scope(|s| {
        ...
        .unwrap();
        s.spawn(|| {
            unsafe { (*slice.get_mut_ptr_unchecked(1)).copy_from_slice(&[42, 69]) };
        })
        .join()
        .unwrap();
    });
